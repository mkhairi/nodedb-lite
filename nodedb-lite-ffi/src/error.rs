...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn nodedb_last_error(_handle: *mut NodeDbHandle) -> *mut c_char {
    ffi_guard_keep_error(std::ptr::null_mut(), || {
        LAST_ERROR.with(|slot| match slot.borrow().as_ref() {
            Some(cs) => {
                // Clone: caller owns this allocation and frees it with
                // nodedb_free_string. The TLS slot stays untouched.
                match CStr::from_bytes_with_nul(cs.to_bytes_with_nul()) {
                    Ok(c) => c.to_owned().into_raw(),
